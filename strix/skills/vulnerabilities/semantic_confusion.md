...
- For two local parsers, canonicalizers, or validator/consumer functions, write a small property harness (paired inputs, bounded examples) and keep the minimized disagreement as a regression test.
- For an ordered HTTP flow with cookies, redirects, captured values, and assertions, script the same request chain against vulnerable, fixed, and negative-control environments.
