"""`lyrashield view [<run>]` command: serve a run's viewer UI locally."""
...
from lyrashield.interface.viewer.transcript import read_run_summary
...
def run_view(argv: list[str]) -> None:
    parser = argparse.ArgumentParser(
        prog="lyrashield view",
        description="Open a local web view of a LyraShield run (live or finished).",
    )
    parser.add_argument(
        "run",
        nargs="?",
    ...
    if not bundle_is_built():
        console.print(
            ...
            "Build it with: [cyan]cd lyrashield/interface/viewer/frontend "
