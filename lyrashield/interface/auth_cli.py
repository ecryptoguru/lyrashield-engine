...
def _status(console: Console) -> int:
    ...
    if record is None:
        console.print(
            "[yellow]Not signed in.[/] Run [cyan]lyrashield auth login chatgpt[/] to sign in."
        )
        return 1
    ...
    if codex.subscription_model(settings.llm.model):
        ...
        console.print(
            "  [yellow]Note:[/] set [cyan]STRIX_LLM[/] to e.g. [cyan]chatgpt/gpt-6-luna[/] "
...
def _print_success(console: Console) -> None:
    ...
    text.append("chatgpt/gpt-6-luna", style="bold cyan")
