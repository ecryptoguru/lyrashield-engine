...
It also supports `chatgpt/gpt-6-*` subscription-backed routes for the main model by default (subscription runs record zero metered token cost). Set `LYRASHIELD_ALLOW_CHATGPT_SUBSCRIPTION=0` to disable the subscription path and require a metered GPT-6 Sol or Luna deployment.
