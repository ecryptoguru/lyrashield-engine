...
def test_resolves_common_bare_model_names() -> None:
    ...
    # The live model-cost map lists a bare ``grok-4.5`` entry resolving to xai;
    # the bundled fallback (offline CI) may lack it, so inject it explicitly
    # rather than depend on which map variant loaded at litellm import.
