"""Tests for lyrashield.artifacts.writer artifact helpers."""
...
from strix.report import writer as strix_writer
...
def test_write_vulnerabilities_csv_neutralizes_formula_injection(
    ...
    strix_writer.write_vulnerabilities(tmp_path, [_sample_report(title=payload)], set())
...
def test_write_vulnerabilities_csv_leaves_benign_titles_unchanged(tmp_path: Path) -> None:
    strix_writer.write_vulnerabilities(
        tmp_path, [_sample_report(title="SQL Injection in /login")], set()
    )
...
def test_atomic_write_text_keeps_payload_byte_for_byte(tmp_path: Path) -> None:
    ...
    strix_writer.atomic_write_text(path, payload)
