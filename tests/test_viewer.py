...
# The substrate viewer keeps upstream's remote-bind and email-verification
# surface; the owned viewer (lyrashield.interface.viewer) is loopback-only with
# session gating. Substrate-only tests use these aliases.
from strix.interface.viewer.cli import _state_label, run_view
from strix.interface.viewer.server import serve as strix_serve
...
def test_server_can_bind_all_ipv4_interfaces(tmp_path: Path) -> None:
    ...
    httpd, url, _ = strix_serve(run_dir, host="0.0.0.0", open_browser=False)
    try:
