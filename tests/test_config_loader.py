...
def test_read_json_overrides_maps_web_search_provider(tmp_path: Path) -> None:
    # The product supports only the ``parallel`` web-search provider (no exa),
    # so STRIX_WEB_SEARCH_PROVIDER maps to the owned ``web_search`` submodel and
    # unknown integrations keys stay unmapped.
    path = tmp_path / "cli-config.json"
    path.write_text(
        json.dumps({"env": {"EXA_API_KEY": "exa-key", "STRIX_WEB_SEARCH_PROVIDER": "parallel"}}),
        encoding="utf-8",
    )
    assert loader._read_json_overrides(path) == {
...
def test_persist_current_empty_primary_alias_does_not_save_sibling(
    ...
    # env_ignore_empty=True (Compose placeholder handling): the empty primary
    # alias is ignored, so the sibling's value resolves.
    assert loader.load_settings().llm.api_key == "sibling-key"

    loader.persist_current()

    # persist_current writes only the active alias: the empty LLM_API_KEY is
    # dropped (env[active] is falsy) and OPENAI_API_KEY is not re-saved.
