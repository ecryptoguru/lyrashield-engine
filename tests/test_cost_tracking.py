...
def test_cost_callback_defers_gpt6_cost_to_the_token_rate_card() -> None:
    report_state = MagicMock()
    response = SimpleNamespace(usage=SimpleNamespace(cost=0.01), _hidden_params={})

    with patch("lyrashield.artifacts.state.get_global_report_state", return_value=report_state):
        litellm_cost_callback({"model": "azure_ai/gpt-6-luna", "response_cost": 0.01}, response)
...
def test_streamed_openrouter_costs_cleared_on_new_run() -> None:
    ...
    set_global_report_state(ReportState(run_name="clear-stale"))
