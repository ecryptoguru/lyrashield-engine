...
def case_matches(case: dict, finding: dict, corpus_dir: Path) -> bool:
...
def score(results_dir: Path, corpus_dir: Path) -> dict:
    ...
    for finding in scan_findings:
        ...
        for case in corpus["pairs"]:
            if case_matches(case, finding, corpus_dir):
    ...
    stability = None
    if engine_runs and manifest["runs"][0].get("runIndex") is not None or len({r.get("runIndex") for r in engine_runs}) > 1:
        ...
        for f in scan_findings:
            if f.get("scanner") == "engine" and "engine-" in (f.get("scanId") or ""):
                ...
                for case in corpus["pairs"]:
                    if case_matches(case, f, corpus_dir):
