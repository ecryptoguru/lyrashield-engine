...
`vulnerabilities.csv` is a spreadsheet presentation export for the supported Excel for Mac workflow. Formula-like cells receive a leading tab so Excel displays them as text. That prefix is part of the CSV data: programmatic consumers requiring canonical values must use `vulnerabilities.json` or SARIF. Other spreadsheet applications and import settings are not covered by this compatibility claim.
