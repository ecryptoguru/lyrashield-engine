...
<Warning>
Local models are not supported by LyraShield Engine. Production accepts only approved GPT-6 Sol or Luna deployments through the documented OpenAI, Azure/Azure AI, or authenticated `chatgpt/` provider boundary. This unlinked page is retained only as operator reference for offline experimentation and to make the inherited upstream documentation history explicit. Do not configure local-model endpoints in the worker or forward local-model credentials to production.
</Warning>

The upstream-derived engine can be pointed at a self-hosted LLM for completely offline, privacy-first experimentation. Data never leaves your machine, which can be useful for sensitive internal networks or air-gapped operator testing. This is not a production path: most local models lack the agentic reliability the GPT-6 Sol/Luna contract requires, and the LyraShield product entry point rejects non-GPT-6 deployments before scan start.

## Privacy vs Performance

| Feature | Local Models | GPT-6 Sol/Luna |
|---------|--------------|--------------------|
