...
| `STRIX_IMAGE_DIGEST` | unset | Optional digest to verify after `pull_docker_image` completes (for example, `sha256:abc123…`). The pull fails if the local image digest does not contain this value. |
