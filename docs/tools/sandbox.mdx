...
The engine starts the sandbox with `NET_ADMIN` and `NET_RAW` capabilities so tools such as `nmap` can perform raw-socket work. If your workflow does not need this surface, set `STRIX_SANDBOX_DISABLE_NETWORK_CAPABILITIES=1` to drop both capabilities from created containers.
