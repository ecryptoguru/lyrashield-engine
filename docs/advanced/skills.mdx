...
Skills are structured knowledge packages that give engine agents task-specific vulnerability, technology, and testing context.

<Warning>
A skill influences model behavior; it does not prove a finding, coverage, or result quality. Evidence state is owned by the application, and quality claims require the LyraShield evaluation corpus.
</Warning>
...
When the engine spawns an agent for a specific task, it selects a bounded set of relevant skills from the context:
...
The skill catalog lives under `strix/skills/` and is part of the controlled derivative. Any change must follow the same review boundary as other upstream files:
...
Good skill contributions include real-world techniques, practical payloads with variations, clear validation steps, and context-aware guidance for specific versions and environments.
