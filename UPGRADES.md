...
- Optional `viewer` extra: `reportlab`/`pypdf` are not base dependencies, since
  the worker never runs the local viewer. `strix.spec` still bundles them, so
  both binary build paths and the verification gate sync `--extra viewer`.
